// vibecodea.js - Juego infinito con reflejo correcto del perrito y controles arcade
const config = {
  type: Phaser.AUTO,
  width: 800,
  height: 600,
...
for(const [arc, keys] of Object.entries(ARCADE_CONTROLS)){
  keys.forEach(k => KEYBOARD_TO_ARCADE[k.toLowerCase()] = arc);
}
...
function create() {
  ...
  // Controles
  cursors = scene.input.keyboard.addKeys({ up: false, down: false, left: false, right: false });
  scene.input.keyboard.on('keydown', event => {
      const key = KEYBOARD_TO_ARCADE[event.key.toLowerCase()] || event.key.toUpperCase();

      if(gameOverFlag && (key === 'P1A' || key === 'START1')) restartGame(scene);

      if(key === 'P1U') cursors.up.isDown = true;
      if(key === 'P1D') cursors.down.isDown = true;
      if(key === 'P1L') cursors.left.isDown = true;
      if(key === 'P1R') cursors.right.isDown = true;
      if(key === 'P1A') attack(scene);
  });
  scene.input.keyboard.on('keyup', event => {
      const key = KEYBOARD_TO_ARCADE[event.key.toLowerCase()] || event.key.toUpperCase();
      if(key === 'P1U') cursors.up.isDown = false;
      if(key === 'P1D') cursors.down.isDown = false;
      if(key === 'P1L') cursors.left.isDown = false;
      if(key === 'P1R') cursors.right.isDown = false;
  });
  ...
  scene.time.addEvent({ delay: monsterSpawnDelay, loop: true, callback: () => { if(monsters.getLength()<maxMonsters) spawnMonster(scene); }});
...
// Funciones de spawn y acción
function attack(scene){
  const now=Date.now();
  if(now-lastAttack<700 || energy<30) return;
  lastAttack=now;
  energy-=30;
  energyText.setText('Energía: '+Math.floor(energy));
  ...
  monsters.getChildren().slice().forEach(m=>{
      ...
      if(d<=60){ m.destroy(); score+=30; scoreText.setText('Puntos: '+score); }
  });
}
...
function spawnBanana(scene){
  ...
  bananas.add(b);
  if(isBomb) scene.physics.add.overlap(player,b,()=>{
      b.destroy();
...
function gameOver(scene){
  ...
  player.body.setVelocity(0);
  scene.time.addEvent({delay:3000, callback:()=>{ restartGame(scene); }});
}

function restartGame(scene){
  lives=3; energy=100; score=0; gameOverFlag=false;
  livesText.setText('Vidas: '+lives);
  energyText.setText('Energía: '+Math.floor(energy));
  scoreText.setText('Puntos: '+score);
  scene.scene.restart();
